...
std::vector<int> DynMeans::updateState(std::vector<int> lbls, std::vector<int> cnts, std::vector<V2d> prms){
	this->oldprms = prms;
...
std::vector<int> DynMeans::cluster_wrapper(std::vector<double> newobservations, int nRestarts){
    vector <V2d> clusterData;
    // assign data to clusterData from newobservations
    for(int j=0; j<newobservations.size(); j=j+2)
    {
        V2d newData(newobservations[j], newobservations[j+1]);
    ...
    double tTaken, obj;
    this->cluster(clusterData, nRestarts, learnedLabels, learnedParams, obj, tTaken);
    return learnedLabels;
}

void DynMeans::cluster(std::vector<V2d>& newobservations, int nRestarts,
		std::vector<int>& finalLabels, std::vector<V2d>& finalParams, double& finalObj, double& tTaken){
	timeval tStart;
...
void DynMeans::assignObservations(std::vector<int> assgnOrdering, std::vector<int>& lbls, std::vector<int>& cnts, std::vector<V2d>& prms){
...
double DynMeans::setParameters(std::vector<int>& lbls, std::vector<int>& cnts, std::vector<V2d>& prms){
	double objective = 0;
	for (int i = 0; i < prms.size(); i++){
		if (cnts[i] > 0){
			...
			std::vector<V2d> obsInClus = this->getObsInCluster(i, lbls);
